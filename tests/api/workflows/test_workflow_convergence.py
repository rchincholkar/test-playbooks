...
class Test_Workflow_Convergence(APITest):
    ...
    convergence_node_parent_node_all_parents_must_converge_test_cases = [
        ConvergenceNodeTestCase(
            ('n0_success',
             'n1_success'),
            'Success-Parents-Always-Relation'),
        ConvergenceNodeTestCase(
            ('n1_success',
             'n2_fail'),
            'Success-And-Failure-Parents-Always-Relation'),
        ...
        ConvergenceNodeTestCase(
            ('n1_success',
             'n3_dnr'),
            'DNR-And-Success-Parents-Failure-Relation'),
    ]
    """
    ...
    @pytest.mark.parametrize(
        'test_case', convergence_node_parent_node_all_parents_must_converge_test_cases,
        ids=[case.case_name for case in convergence_node_parent_node_all_parents_must_converge_test_cases])
    def test_convergence_node_runs_when_all_parents_must_converge_is_set_to_true(
            self, skip_if_openshift, factories, test_case):
        """Confirm that convergence node job runs in the following scenarios
        ...
        n0 (uses jt that succeeds)
        n1: (uses jt that succeeds)
        n2: (uses jt that fails)

        Parameterized values:
            Each test case tests a different combination of "parent" nodes for the convergence node.

        Expect:
         - n0 should run and succeed
         - n1 should run and succeed
         - n2 should run and fail
         - convergence_node should run and succeed
        ...
        if 'n0_success' in test_case.parent_nodes:
            parent_nodes.append((n0, 'success parent'))
            with pytest.raises(NoContent):
                # this returns a 204, which raises an exception in awxkit
                # because it is not 200 but we are OK with it in this context
                n0.related.success_nodes.post(dict(id=convergence_node.id))
        if 'n1_success' in test_case.parent_nodes:
            parent_nodes.append((n1, 'success parent'))
            with pytest.raises(NoContent):
                n1.related.success_nodes.post(dict(id=convergence_node.id))
        if 'n2_fail' in test_case.parent_nodes:
            parent_nodes.append((n2, 'failure node parent'))
            with pytest.raises(NoContent):
                n2.related.failure_nodes.post(dict(id=convergence_node.id))
        ...
        assert 'successful' == get_job_status(wfj, n0.id, mapping)
        assert 'successful' == get_job_status(wfj, n1.id, mapping)
        assert 'failed' == get_job_status(wfj, n2.id, mapping)
        assert 'successful' == get_job_status(
            wfj, convergence_node.id, mapping)
